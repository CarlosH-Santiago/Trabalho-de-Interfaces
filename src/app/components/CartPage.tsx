...
export function CartPage({ onBack }: CartPageProps) {
  ...
  if (cartItemCount === 0) {
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center justify-center p-8">
        ...
        <p className="text-zinc-500 mb-8">Parece que você ainda não escolheu nenhuma peça exclusiva.</p>
        <button 
          onClick={onBack}
          className="bg-white text-black px-8 py-3 uppercase tracking-widest text-sm hover:bg-zinc-200 transition-colors"
        >
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-8 md:p-16">
      <div className="max-w-6xl mx-auto">
        ...
        <h1 className="text-3xl tracking-widest uppercase mb-12 border-b border-zinc-800 pb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            {cart.map((item) => (
              <div key={item.id} className="flex gap-6 bg-zinc-900 p-4 rounded-lg items-center border border-zinc-800">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-zinc-400 mt-1">R$ {item.price.toFixed(2)}</p>
                  
                  {/* Microinterações de quantidade */}
                  <div className="flex items-center gap-4 mt-4">
                    ...
                    <button onClick={() => removeFromCart(item.id)} className="text-zinc-500 hover:text-red-500 transition-colors p-2">
                      <Trash2 size={20} />
                    </button>
                  </div>
                </div>
          ...
          <div className="bg-zinc-900 p-8 rounded-lg border border-zinc-800 h-fit">
            ...
            <div className="space-y-4 text-zinc-400 border-b border-zinc-800 pb-6 mb-6">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white uppercase text-xs tracking-widest">Grátis</span>
              </div>
            </div>
            <div className="flex justify-between text-xl font-medium mb-8 text-white">
              <span>Total</span>
              <span>R$ {cartTotal.toFixed(2)}</span>
            </div>
            
            <button className="w-full bg-white text-black py-4 uppercase tracking-widest text-sm hover:bg-zinc-200 transition-colors font-medium">
              Finalizar Compra
            </button>
          </div>
